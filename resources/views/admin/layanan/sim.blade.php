                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50 border-b border-gray-100 text-gray-500 font-inter text-sm">
                                ...
                                <th class="p-4 font-semibold">Warga</th>
                                <th class="p-4 font-semibold">Jenis SIM</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr class="hover:bg-gray-50/50 transition">
                                ...
                                <td class="p-4">
                                    ...
                                    <p class="text-xs text-gray-500">NIK: {{ $item->nik }}</p>
                                </td>
                                <td class="p-4 font-bold text-primary">SIM {{ $item->jenis_sim }}</td>
                                <td class="p-4 text-sm text-blue-500">
                                    <div class="flex flex-col gap-1">
                                        <a href="{{ asset('storage/' . $item->ktp) }}" target="_blank" class="hover:underline">KTP</a>
                                        <a href="{{ asset('storage/' . $item->pas_foto) }}" target="_blank" class="hover:underline">Foto</a>
                                        <a href="{{ asset('storage/' . $item->surat_kesehatan) }}" target="_blank" class="hover:underline">Kesehatan</a>
                                        <a href="{{ asset('storage/' . $item->surat_psikologi) }}" target="_blank" class="hover:underline">Psikologi</a>
                                    </div>
                                </td>
                                <td class="p-4">
                                ...
                                <td class="p-4">
                                    <form action="{{ route('admin.sim.update', $item->id) }}" method="POST" class="flex flex-col gap-2 min-w-[200px]">
                                        @csrf
                                        @method('PATCH')
                            ...
                            @empty
                            <tr>
                                <td colspan="7" class="p-8 text-center text-gray-500">Belum ada pengajuan registrasi SIM.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
